...
namespace eTopics.Service {
    public class PostService : IPostService {
        ...
        public IEnumerable<Post> GetPosts(ApplicationUsers applicationUser) {
            return applicationDbContext.Posts
                .Include(post => post.Creator)
        ...
        public async Task<Post> Delete(Post post, int postid)
        {
            post = applicationDbContext.Posts.FirstOrDefault(u => u.Id == postid);
            if (post != null)
            {
                applicationDbContext.Database.ExecuteSqlRaw("DELETE FROM COMMENTS WHERE PostId=@postid", new SqlParameter("@postid", postid));
